...
//! fixpoint:
//!
//!  - Unit propation: if there is a clause with just one literal, it must be
...
impl Preprocessing {
    pub fn new(var_count: u32, clauses: Vec<Clause>) -> Self {
        ...
        for var_id in 0..var_count {
            ...
            } else if counts[!lit] == 0 && counts[lit] != 0 {
                pure_literal_worklist.push(!lit);
            }
        }
        Self {
    ...
    pub fn simplify(&mut self) {
        // TODO: apply preprocessing steps to a fixpoint
...
impl Preprocessing {
    ...
    fn process_one_pure_lit(&mut self, lit: Literal) {
        ...
        // Future TODO: use try_insert().unwrap() if/when it's stabilized
        let old_entry = self.solutions.insert(
            lit.var_id(),
            if lit.is_negated() {
                Soln::False
            } else {
    ...
    fn one_unit(&mut self, lit: Literal, worklist: &mut Vec<Literal>) {
        // TODO: check if literal has been assigned yet,
        // and do the right thing based on it
        // Otherwise, assign it to true
        todo!();
        ...
        for clause_ind in (0..self.clauses.len()).rev() {
            ...
            if self.clauses[clause_ind].len() <= 1 {
                debug_assert!(!self.clauses[clause_ind].is_empty());
